...
function RegistrationForm() {
  ...
  const token = user?.token; // Obtén el token del contexto
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Enviar datos al backend (incluye token en headers si es necesario)
      const response = await fetch("/api/students", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // Si la ruta está protegida, puedes agregar el token:
      ...
      if (!response.ok) {
        ...
        throw new Error(
          "Error en el registro: " + (errorData.error || "Respuesta no exitosa")
        );
      }
      // Resetear formulario
